...
public class FCMService {
    ...
    public String sendMessageWithoutData(PushNotificationRequest request)
            throws InterruptedException, ExecutionException {
    ...
    private ApnsConfig getApnsConfig(String topic) {
        return ApnsConfig.builder()
                .setAps(Aps.builder().setThreadId(topic).build()).build(); // .setCategory(topic)
    }

    private Message getPreconfiguredMessageToToken(PushNotificationRequest request) {
        return getPreconfiguredMessageBuilder(request).putAllData(request.getMap()).setToken(request.getToken())
                .build();
    }
